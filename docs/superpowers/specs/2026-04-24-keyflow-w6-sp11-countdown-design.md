...
| `Assets/Editor/SceneBuilder.cs` | Add private `BuildCountdownOverlay(Camera, AudioClip clickSample, GameObject pauseButtonRoot, Transform gameplayRoot)` method. Call site: after the existing `var hudPauseButton = BuildHUD(...)` line (line 115 today) — pass `hudPauseButton.gameObject` as `pauseButtonRoot`. `BuildHUD` already returns `HUDPauseButton` so no plumbing change needed. Wire `countdown` SerializeField onto `GameplayController` via existing `SetField` helper. Scene save: `countdownCanvas.SetActive(false)` to match SP10 pattern. |
...
| 3 | `Tick_AlphaFadeAfterFadeStart` | Given `Activate(startTime=0, lifetime=1.0, ...)`: `TickForTest(0.54)` → alpha == 1.0 (fadeStart=0.55 hasn't kicked in). `TickForTest(0.99)` → alpha ≈ 0.022 (t≈1.0 end of fade curve). `TickForTest` is absolute time; normalized `t = (simulatedTime - startTime) / lifetime`. |
...
| Popup Update-driven `SetActive(false)` vs coroutine `onComplete` same-frame race | Low | Visual flicker | Coroutine owns the "hide then invoke onComplete" ordering explicitly. Sequence: yield `goHoldSec` → popup already self-deactivated by its own Update (`popupLifetimeSec=0.9 < goHoldSec + stepDurationSec = 1.4`) → restore pauseButton → invoke `onComplete`. No contested frame. |
