...
public void HandleTap_FiresFeedbackEvent_OnMiss()
{
    ...
    // Normal difficulty Good window max is 100ms (per JudgmentEvaluator), so +200ms → Miss.
...
namespace KeyFlow.Tests.EditMode
{
    public class FeedbackDispatcherTests
    {
        ...
        [Test]
        public void ReceivesCorrectJudgmentKind_ForAllFourViaHoldBreakAndAutoMiss()
        {
            UserPrefs.HapticsEnabled = true;
            var (js, d, h, p) = Build();
            var note = MakeNoteAt(Vector3.zero);

            js.HandleAutoMiss(note);                // Miss
            js.RegisterPendingNote(MakeNoteAt(Vector3.zero));
            js.InvokeHandleTapForTest(1000, 0);     // Perfect (note at hit=1000, tap at 1000 if note was at lane 0… build specifically)

            // This test only validates Miss explicitly via HandleAutoMiss;
            // Perfect/Great/Good coverage is exercised in JudgmentSystemTests.
            Assert.IsTrue(p.calls.Count >= 1);
            Assert.AreEqual(Judgment.Miss, p.calls[0].j);
...
Add a new private static method to `SceneBuilder`:
    ...
    JudgmentSystem judgmentSystem, Transform parent)
{
    ...
    // HapticService — assigns presets SO via SerializedObject
    ...
    SetPrivateField(hapticService, "presets", presets);
    ...
    SetPrivateField(particlePool, "hitPrefab", hitPrefab);
    SetPrivateField(particlePool, "missPrefab", missPrefab);
    SetPrivateField(particlePool, "presets", presets);

    // FeedbackDispatcher — wires the above two + judgmentSystem
    ...
    SetPrivateField(dispatcher, "judgmentSystem", judgmentSystem);
    SetPrivateField(dispatcher, "hapticService", hapticService);
    SetPrivateField(dispatcher, "particlePool", particlePool);
}
...
Add these `using` directives at the top of `SceneBuilder.cs`:
...
Open `Assets/Editor/SceneBuilder.cs`, find `BuildSettingsCanvas`. Inside it, after the sliders are built (follow existing `BuildSlider` or `BuildButton` helper patterns in the file), add a Toggle. Because the existing file structure isn't fully visible in this plan, the implementer should:

1. Look at how `sfxVolumeSlider` is created in `BuildSettingsCanvas`.
2. Add a similar block creating a `Toggle` GameObject with a label "Haptics" and wiring it to the `SettingsScreen.hapticsToggle` SerializeField via the same `SetPrivateField` helper from Task 9.

Skeleton (adapt to existing style):

```csharp
var hapticsGo = new GameObject("HapticsToggle", typeof(RectTransform));
hapticsGo.transform.SetParent(settingsPanel.transform, false);
var toggle = hapticsGo.AddComponent<Toggle>();
// ... layout/label styling consistent with existing sliders
SetPrivateField(settingsScreen, "hapticsToggle", toggle);
```

If the existing `BuildSettingsCanvas` is terse and adding the full Toggle here is involved, prefer **Option B**: expose a runtime menu item to enable/disable haptics via PlayerPrefs directly (defer UI to follow-up SP). **Not recommended** — the spec explicitly wants a Settings toggle.
