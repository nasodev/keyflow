...
2. In `HoldTracker.Update()`, extend the early-return guards so audio retrigger runs whenever holdAudio has entries, but **without bypassing the existing IsPlaying / IsPaused guards**:
   ...
   ```
   **Safe-mutation note**: mutating a Dictionary's keyset (add/remove) during iteration invalidates the struct enumerator. We're only overwriting a value for a key that already exists via the indexer setter. Empirically in current Mono and IL2CPP runtimes, `Dictionary<K,V>` does not bump its version counter on a value-only indexer set on an existing key, so iteration continues safely. **Note: this is distinct from `HoldStateMachine.Tick`'s `e.state = ...` pattern** — `HoldStateMachine.Entry` is a class (reference type), so that code mutates the referenced object and never writes back to the dictionary; it's not the same primitive. If the indexer-set-during-iteration behavior tightens in a future runtime (tracked by .NET team discussions periodically), the fallback spelled out in the Risks table applies: maintain a `List<int> retriggerBuffer` field, collect pending keys during iteration, and flush value writes after the loop. No API change required.
...
| Struct-value write during Dictionary iteration triggers version-check exception | Low | Medium (runtime exception under load) | Empirically, Mono/IL2CPP's `Dictionary<K,V>` does not bump its version counter on a value-only indexer set for an existing key, so mid-foreach indexer writes are safe. (Distinct from `HoldStateMachine.Tick`'s pattern, which mutates a class field via reference and never touches the dictionary.) If a future runtime tightens this, fall back to a `List<int> retriggerBuffer` field that collects pending keys and flushes after the loop (no API change). |
