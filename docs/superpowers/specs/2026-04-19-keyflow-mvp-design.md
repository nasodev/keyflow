...
수록 곡은 **5곡**으로 시작한다. 전부 퍼블릭 도메인 클래식이며 자체 MIDI 시퀀싱 + Salamander Grand Piano(CC-BY 3.0, Alexander Holm) 샘플러로 렌더링한다.
...
- 샘플레이트: 48000 Hz 고정
- 피아노 음색: Salamander Grand Piano V3 (CC-BY 3.0, Alexander Holm), 88건반 중 MVP에서 48건반(MIDI 36~83)만 OGG로 사전 인코딩 → ~15MB
...
3. Salamander Piano V3 다운로드 (CC-BY 3.0)
